...
namespace DockerWizard
{
    internal class Program
    {
        ...
        private static async Task Main()
        {
            ...
            {
                Console.WriteLine("\n\n[ Local commands ]\n");
                RunCreateAndPushCommands(_filePocketCommands!.CreateAndPushCommands!);
        ...
        private static void RunCreateAndPushCommands(List<DockerCommand> commands)
        {
            foreach (var c in commands)
            {
                Console.WriteLine(c.Name);
                _dockerService!.RunLocalCommand(c.Command, _filePocketRootFolder!);
                Console.WriteLine();
            }
        }
        ...
        private static async Task PullAndDeploy(List<DockerCommand> commands)
        {
            ...
            {
                Console.WriteLine(c.Name);
                var result = await _dockerService!.RunRemoteCommand(c.Command);
